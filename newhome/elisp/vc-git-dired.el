...
(defun vc-git-dired-whatsnew (arg)
  ...
  (display-buffer "*vc*" 'display-buffer-use-some-window))
...
(defun git-status-porcelain (buffer directory)
  ...
  (with-current-buffer (get-buffer-create buffer)
    ...
    (let ((default-directory directory)
	  (s0 (buffer-string)))
      (vc-git-command buffer 0 nil "status" "-b" "--porcelain" "-u" "--ignored" ".")
...
; (git-status-porcelain (get-buffer-create "*vc*") (dired-current-directory))
